...
export async function POST(request: NextRequest) {
  try {
    ...
    // Extract the matching virtual account generated by PocketFi for this checkout session
    const virtualAccount = checkoutSession.virtualAccount || {
      bank_name: 'SafeHaven MFB',
      account_number: checkoutSession.paymentId?.replace(/^PFI\|/, '') || '',
      account_name: `ADISION / ${title.slice(0, 14)}`,
