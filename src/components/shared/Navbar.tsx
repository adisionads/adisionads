...
export function Navbar() {
  ...
  const { currentRole, setCurrentRole, wallet } = useApp();
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-slate-200/80 dark:border-slate-800/80 bg-white/85 dark:bg-dark-900/80 backdrop-blur-xl transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        {/* Right Section: Theme Toggle, Role Switcher & User Actions */}
        <div className="hidden md:flex items-center gap-3">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700/80 bg-slate-100 dark:bg-slate-800/60 hover:bg-slate-200 dark:hover:bg-slate-800 text-xs font-semibold text-slate-800 dark:text-slate-200 transition-colors shadow-inner"
            >
              <span className="w-2 h-2 rounded-full bg-brand-500 animate-pulse" />
              <span className="text-slate-500 dark:text-slate-400">View as:</span>
              <span className="text-brand-600 dark:text-brand-400 uppercase font-bold">
                {currentRole === 'ADVERTISER' ? 'Advertiser' : currentRole === 'COMMUNITY_PARTNER' ? 'Partner' : 'Admin'}
              </span>
              <ChevronDown className="w-3.5 h-3.5 text-slate-400" />
            </button>

            {roleDropdownOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-2 z-50 animate-in fade-in zoom-in-95">
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-medium text-left transition-colors ${
                    currentRole === 'ADVERTISER'
                      ? 'bg-brand-50 dark:bg-brand-500/10 text-brand-700 dark:text-brand-400 border border-brand-200 dark:border-brand-500/20 font-bold'
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-slate-500 dark:text-slate-400">Create ads, buy reach</div>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-slate-500 dark:text-slate-400">Publish ads, earn wallet cash</div>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-slate-500 dark:text-slate-400">Matchmaking, verify proofs</div>
                  </div>
                </button>
              </div>
            )}
        ...
        <div className="flex items-center gap-2 md:hidden">
          ...
          <button
            onClick={() => setRoleDropdownOpen(!roleDropdownOpen)}
            className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-[11px] font-semibold text-brand-600 dark:text-brand-400 border border-slate-200 dark:border-slate-700"
          >
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 px-4 py-5 space-y-4 animate-in slide-in-from-top duration-200">
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-slate-800">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 uppercase">Switch View:</p>
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                className={`p-2 rounded-xl text-xs font-semibold ${
                  currentRole === 'ADVERTISER' ? 'bg-brand-500 text-dark-900 font-bold' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'
                }`}
              >
                Advertiser
              </button>
              <button
                ...
                className={`p-2 rounded-xl text-xs font-semibold ${
                  currentRole === 'COMMUNITY_PARTNER' ? 'bg-brand-500 text-dark-900 font-bold' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'
                }`}
              >
                Partner
              </button>
              <button
                ...
                className={`p-2 rounded-xl text-xs font-semibold ${
                  currentRole === 'ADMIN' ? 'bg-brand-500 text-dark-900 font-bold' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'
                }`}
              >
                Admin
              </button>
